...
| 模块 | 技术选型 | 说明 |
| --- | --- | --- |
| LLM | Qwen / DashScope OpenAI-compatible API | 用于 Orchestrator 结构化规划、DataAnalyst SQL 生成、结果直答和 DecisionMaker 建议生成。系统要求模型不可用时明确报错，不使用本地假建议伪装成功。 |
| Agent 框架 | LangGraph `StateGraph` | 将协调器、数据分析、预测、可视化、反事实模拟、决策建议组织为有状态执行图，通过条件边控制预测和 What-if 分支。 |
| 查询引擎 | MySQL| 基础表与预聚合视图共同驻留数据库|
...
对月度 GMV、州销售排行、配送延迟三个高频问题分别执行基础表 SQL 和预聚合 SQL：

每个问题分别执行“基础表 JOIN + GROUP BY”版本和“预聚合表查询”版本，统计查询耗时、返回行数和加速倍数。
...
除关键词统计外，系统还在 ETL 阶段对 `order_reviews` 中 `review_score<=2` 的负面评论文本进行 TF-IDF + NMF 无监督主题建模，结果落地为 `mv_review_topics`。这一设计用于补足简单关键词分类的不足。

原 `mv_review_category_perf` 使用葡萄牙语关键词 `LIKE` 将差评粗分为物流、质量、错发、客服和其他。但 Olist 评论存在两个问题：一是大量订单没有评论文本，二是葡萄牙语表达方式较多，固定关键词难以覆盖真实投诉原因。因此仅依赖关键词会让大量差评落入“其他”，难以支撑经营决策。

`mv_review_topics` 的字段包括 `product_category_name`、`topic_id`、`topic_label`、`topic_keywords`、`complaint_count` 和 `topic_share`，并包含 `ALL` 平台级主题行。模型本地训练，无需下载预训练模型；运行时 Agent 只查询预聚合结果，不增加在线问答负担。

实测主题能够揭示关键词分类遗漏的原因，例如：

- `comprei dois · recebi apenas` 对应“付款后未收到货 / 漏发缺件”。
- `compra · pedido · dia` 对应“下单后等待时间长 / 物流拖延”。

DataAnalyst 将 `mv_review_topics` 作为差评原因诊断证据，DecisionMaker 再据此输出履约漏发、物流提速、质检和售后流程优化建议，形成“NLP 分析 -> 决策建议”的闭环。

### 5.6 What-if 反事实模拟

系统新增 WhatIf 反事实模拟节点，用于回答“如果采取某项运营动作，平台指标会怎样变化”。当用户问题出现“如果、假设、下架、移除”等反事实意图时，LangGraph 条件边会触发 `whatif_model` 节点。该节点先将自然语言假设映射到预置场景，再在真实订单和评价数据上重算干预前后指标。

当前已落地两个场景：

- 下架评分最低的 Top-N 卖家后，平台整体平均评分如何变化。
- 如果消除所有延迟订单，平台整体平均评分如何变化。

计算过程保持确定性和可复现：基线和反事实来自同一批真实评价，差异只来自“排除哪一批订单/评价”，不会生成模拟数据。以下架最差卖家为例，系统先从 `mv_seller_perf` 中毫秒级选出风险卖家，仅在重算平台评分时下钻基础表，用一趟扫描同时计算干预前后均分。

实测结果显示，**下架最差 20 个卖家，平台均分约从 4.142 提升到 4.145，仅提升 +0.003，影响 181 条评价；而消除配送延迟，平台均分约从 4.142 提升到 4.283，提升 +0.141，影响约 8.1% 的评价**。这说明平台体验问题更可能来自系统性物流履约，而不是少数长尾卖家。因此，运营优先级应更偏向高延迟区域履约改善，而不是简单大规模下架卖家。
...
| 成员 | 主要工作 | 比例 |
| --- | --- | ---: |
| 刘继业 | 数据层、MySQL 建表导入、DataAnalyst、SQL 规划、预聚合视图、性能对比脚本、数据校验文档 | 33% |
| 李航 |Orchestrator、LangGraph 多 Agent 调度、会话记忆和 WebSocket 事件流 、视图优先查询策略、基础表回退机制，DecisionMaker、评论主题建模、What-if 模拟 | 33% |
| 欧宇轩| Visualizer、Web Dashboard、Plotly/Folium 图表、前端交互、ForecastModel、DecisionMaker、报告整理| 33% |
