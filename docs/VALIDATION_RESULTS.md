...
## 2026-06-04 What-if 反事实模拟（加分项）

- 新增 `utils/whatif.py` 确定性反事实引擎 + `agents/orchestrator.py` 的 `whatif_model` Agent 节点：协调器识别"如果/假设/下架/移除"等意图后由条件边触发，大模型把自然语言假设映射到预置场景，再在真实订单数据上确定性重算，输出"干预前→干预后"对比，结果回灌 DecisionMaker。
- 两个场景实测形成数据驱动对比：**下架评分最低的 Top20 卖家，平台均分仅 4.142→4.145（+0.003，影响 181 条评价）；消除所有延迟订单，均分 4.142→4.283（+0.141，影响 8.1% 评价）**，据此给出"问题在系统性物流而非个别长尾卖家"的优先级建议。
- 真实单题验收：`python cli.py "如果把Top20高差评卖家的商品全下架，平台整体评分预计提升多少？请给出运营建议。"`，Planner 把 `whatif_model` 写入 required_agents 与步骤（confidence 0.92），`whatif` 载荷含基线/反事实/delta 与 20 个最差卖家明细，直答与三条建议均整合了反事实结论。
- 性能：最差卖家取自预聚合 `mv_seller_perf`（毫秒级），反事实重算用一趟扫描；建库末尾执行 `ANALYZE` 收集统计信息，修正了 SQLite 缺统计时为多表 JOIN 临时自建索引、致查询退化到分钟级的问题。反事实仅在显式假设性问题时触发，不影响常规问答。
- 离线单元验收：`python -m pytest -q tests/test_skeleton.py` 全部 38 项通过（新增反事实意图检测、两场景计算、整链路路由四项测试）。
